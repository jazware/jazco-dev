---
layout: post
title: "When Imperfect Systems are Good, Actually"
...
Often when designing and building systems, we aim for perfection in things like consistency of data, availability, latency, and more.
...
I recently made some major tradeoffs in the design of [Bluesky's](https://bsky.app/) Following Feed/Timeline to improve the performance of writes at the cost of consistency in a way that doesn't meaningfully affect users but reduced P99s by over 96%.

## Timeline Fanout

When you make a post on Bluesky, your post is indexed by our systems and persisted to a database where we can hydrate it on-demand to serve in API responses.

Additionally, your post is "fanned out" to your followers so they can see it in their Timelines.
...
When a user loads their Timeline, we fetch a page of post references and then hydrate the posts/actors concurrently to quickly build an API response and let you see the latest content from people you follow.

The Timelines table is sharded by user meaning each user gets their own Timelines partition, randomly distributed among shards of our horizontally scalable database (ScyllaDB), replicated across multiple shards for availability.
...
Generally, this can be dealt with via policy and moderation to prevent abusive users from causing undue load on systems, but these processes take time and can be imperfect.
...
This additional load slows down the individual operations to the user's Timeline, which is fine for the bad behaving user, but causes problems to the tens of thousands of other users sharing a shard with them.

We typically call this situation a "Hot Shard", where some resident of a shard has "hot" data that is being written to or read from at much higher rates than "normal". Since the data on the shard is only replicated a few times, we can't effectively leverage the horizontal scale of our database to process all this additional work.
...
Under normal circumstances, writing to a single Timeline takes an average of ~600 microseconds. If we sequentially write to the Timelines of our user's followers, we'll be sitting around for 20 minutes to Fanout this post at best.

If instead we concurrently Fanout to 1,000 Timelines at once, we can complete this Fanout job in ~1.2 seconds.

That sounds great, except it's oversimplifying an important property of systems: [tail latencies](https://web.archive.org/web/20200603133348/https://robertovitillo.com/why-you-should-measure-tail-latencies/).
...
Imagine a user who follows hundreds of thousands of others. Their Timeline is being written to several times a second, moving so fast it would be humanly impossible to keep up with the entirety of their Timeline even if it was their full-time job.

For a given user, there's a threshold beyond which it is _unreasonable_ for them to be able to keep up with their Timeline. Beyond this point, they consume content through various other feeds and do not primarily use their Following Feed.

Additionally, beyond this point, it is _reasonable_ for us to provide enough content to consume that the Timeline always has something new, but not necessarily have a perfect chronology of everything posted by the many thousands of users they follow.
...
What if we introduced a mechanism to reduce the correctness of a Timeline such that there is a limit to the amount of work a single Timeline can place on a DB shard.
...
In this case, just before writing a page in Fanout, we can generate a random float between 0 and 1, then compare it to the `loss_factor` of each user in the page. If the user's `loss_factor` is smaller than the generated float, we filter the user out of the page and don't write to their Timeline.

In this way, on average, users all have the same number of "follows worth" of Fanout. For example with a `reasonable_limit` of 2,000, a user who follows 4,000 others will have a `loss_factor` of `0.5` meaning half the writes to their Timeline will get dropped. For a user following 8,000 others, their loss factor of `0.25` will drop 75% of writes to their Timeline.

Thus, each user has a effective ceiling on the amount of Fanout work they can cause to their Timeline.

By examining the limits of _reasonable_ user behavior and embracing imperfection for users who go beyond it, we can continue to provide service that meets the expectations of users without sacrificing scalability of the system.
...
In fact, there now appear to be no hot shards in the cluster at all, and the P99 of Timeline writes has been reduced by over 90%.
...
By caching values here which don't need to be perfect representations of the actual number of users someone follows to function correctly in this case, we can once again embrace imperfection in the system to improve performance and scalability without compromising the function of the service.
